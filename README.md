...
To apply environment variables to every container an agent runs without configuring each pipeline, see [`BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT`](#buildkite_docker_default_environment).
...
The variables in this section are read from the agent's own environment rather than from the plugin's configuration. They let an agent apply settings to every container this plugin runs, without each pipeline having to opt in. They are not plugin options, so they can not be set in a step's `plugins` block — export them from an agent `environment` or `pre-command` hook, or from the agent's own environment.

Each is a single string of `;`-delimited entries. Every entry is trimmed of leading and trailing whitespace and empty entries are skipped, so leading, trailing and repeated `;` are harmless. That makes the append idiom safe when several hooks each want to contribute:

```bash
export BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT="MY_TAG=value; ${BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT:-}"
```

If the variable is unset or empty, nothing is added.

### `BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT`

A `;`-delimited list of environment variables to pass into every container. Each entry becomes a Docker CLI `--env` parameter, and is passed through verbatim, so entries can be specified as either `KEY` or `KEY=value` — exactly as for the [`environment`](#environment-optional-array) option. Entries given as a bare variable name are passed through from the agent's environment; whitespace inside a value is preserved and each entry reaches docker as a single argument.

```bash
#!/bin/bash
# An agent environment hook
set -euo pipefail

export BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT="HTTP_PROXY; HTTPS_PROXY; CI_FLEET=prod-us-east; ${BUILDKITE_DOCKER_DEFAULT_ENVIRONMENT:-}"
```

These `--env` parameters are appended after the ones generated by the step-level `environment` option. Docker resolves a repeated `--env KEY` in favour of the last occurrence, so when a step and the agent both set the same key the agent's value wins — which is what makes this usable to enforce a value across a fleet.
